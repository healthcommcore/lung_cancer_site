...
$fieldName = array("partID", "ixModality",  "dateIXChange", "reasonIX", "smoked", "numCigs","webPwd", "reenterPwd", "remindRand", 
                  "reminModality", "dateRemChang", "reasonRemind", "ptRPhone", "dateRemOpt", "reasonRemOpt");
$cellName = array("Study ID", "Ix Modality", "Date Switched to Print", "Reason Switch to Print", "Smoked in last 30 days", "Number of cigarettes smoked",
...
if($_POST["isSubmittedEnroll1"]!=1){
	...
	for($a = 0; $a <count($cellName); $a++){
		...
		// END $a < 4
			...
			if($a%2 == 0){
				echo "<tr>\n";
				// SMOKING ADDITION - FORM NOT SUBMITTED
				...
				*/
				else if($cellName[$a] == "Reminder Randomization"){
					echo "<td align = \"left\">".$cellName[$a]." <br>\n";
					echo "<select name = \"".$fieldName[$a]."\">\n";
	...
	for($a = 0; $a <count($cellName); $a++){
		if($a == 0){
			...
			if($a%2 == 0){
				echo "<tr>\n";
				// SMOKING ADDITION - FORM SUBMITTED
				if($fieldName[$a] == 'smoked'){
					echo "<td>" . $cellName[$a] . "<br /><select name= \"" . $fieldName[$a] . "\">\n";
